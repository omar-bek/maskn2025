            ...
            <form action="{{ route('client.profile.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-8 border-b border-gray-100 bg-gray-50/50">
                    ...
                    <div class="flex flex-col sm:flex-row gap-6 items-center">
                        @php
                            $pathEdit = $user->avatar_url ?? optional($user->profile)->avatar_url;
                            $avatarUrlEdit = null;
                            if ($pathEdit) {
                                $avatarUrlEdit = str_starts_with($pathEdit, 'http') ? $pathEdit : asset('storage/' . str_replace('public/', '', $pathEdit));
                            }
                        @endphp
                        <div class="relative group">
                            <div class="w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-lg ring-1 ring-gray-100">
                                @if ($avatarUrlEdit)
                                    <img src="{{ $avatarUrlEdit }}" alt="{{ $user->name }}" class="w-full h-full object-cover transition duration-300 group-hover:scale-110">
                                @else
