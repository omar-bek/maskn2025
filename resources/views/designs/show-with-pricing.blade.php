...
@section('content')
    <div class="container mx-auto px-4 py-8 mt-40">
                    ...
                    <div class="border-b border-gray-200 pb-6 mb-6">
                        ...
                        <div class="flex flex-wrap gap-x-6 gap-y-3 text-base text-gray-700">
                            <span class="flex items-center"><i
                                    class="fas fa-palette text-[#f3a446] ml-2 mr-2"></i><strong>{{ __('app.design_show.style') }}</strong><span
                                    class="mr-1">{{ $design->style }}</span></span>
                            <span class="flex items-center"><i
                                    class="fas fa-ruler-combined text-[#f3a446] ml-2 mr-2"></i><strong>{{ __('app.design_show.area') }}</strong><span
                                    class="mr-1">{{ $design->area }}
                                    {{ __('app.design_show.area_unit') }}</span></span>
                            <span class="flex items-center"><i
                                    class="fas fa-map-marker-alt text-[#f3a446] ml-2 mr-2"></i><strong>{{ __('app.design_show.location') }}</strong><span
                                    class="mr-1">{{ $design->location }}</span></span>
                            <span class="flex items-center"><i
                                    class="fas fa-layer-group text-[#f3a446] ml-2 mr-2"></i><strong>{{ __('app.design_show.floors') }}</strong><span
                                    class="mr-1">{{ $design->floors }}</span></span>
                            <span class="flex items-center"><i
                                    class="fas fa-bed text-[#f3a446] ml-2 mr-2"></i><strong>{{ __('app.design_show.bedrooms') }}</strong><span
                                    class="mr-1">{{ $design->bedrooms }}</span></span>
                            <span class="flex items-center"><i
                                    class="fas fa-bath text-[#f3a446] ml-2 mr-2"></i><strong>{{ __('app.design_show.bathrooms') }}</strong><span
                                    class="mr-1">{{ $design->bathrooms }}</span></span>
                        </div>
                    </div>
                    ...
                    @if ($design->description)
                        <div class="mb-6">
                            <h3 class="text-xl font-bold text-gray-800 mb-3 flex items-center">
                                <i
                                    class="fas fa-info-circle text-[#2f5c69] mr-2"></i>{{ __('app.design_show.description_title') }}
                            </h3>
                    ...
                    @if ($design->features)
                        ...
                        @if (is_array($features) && count($features) > 0)
                            <div class="mb-6">
                                <h3 class="text-xl font-bold text-gray-800 mb-3 flex items-center">
                                    <i
                                        class="fas fa-star text-[#2f5c69] mr-2"></i>{{ __('app.design_show.features_title') }}
                                </h3>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div
                                class="text-center bg-gray-50/50 border border-gray-200/60 rounded-xl p-4 transform transition-all duration-300 hover:scale-105 hover:shadow-md">
                                <div class="text-3xl font-bold text-[#f3a446]">
                                    {{ $design->area > 0 ? number_format(($totalAmount ?? 0) / $design->area, 2) : 0 }}
                                </div>
                                <div class="text-sm font-medium text-gray-600 mt-2">
                            ...
                            <div
                                class="text-center bg-gray-50/50 border border-gray-200/60 rounded-xl p-4 transform transition-all duration-300 hover:scale-105 hover:shadow-md">
                                <div class="text-3xl font-bold text-[#2f5c69]">
                                    {{ isset($categoryTotals) && $categoryTotals ? $categoryTotals->count() : 0 }}
                                </div>
                                <div class="text-sm font-medium text-gray-600 mt-2">
            ...
            <div class="overflow-x-auto rounded-lg border border-gray-200/60">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @if (isset($categoryTotals) && $categoryTotals)
                            @foreach ($categoryTotals as $categoryTotal)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 text-center text-sm text-gray-900">
                                        {{ ($totalAmount ?? 0) > 0 ? number_format(($categoryTotal->total / ($totalAmount ?? 1)) * 100, 1) : 0 }}%
                                    </td>
                                </tr>
                            @endforeach
                        @endif
        ...
        @if (isset($itemsByCategory) && $itemsByCategory && $itemsByCategory->count() > 0)
            <div class="space-y-8">
                @foreach ($itemsByCategory as $categoryName => $items)
                    <div
                        ...
                        <div class="bg-gradient-to-r from-[#2f5c69] to-[#1a262a] px-6 py-4 border-b border-[#f3a446]/50">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full">
                                ...
                                <tfoot class="bg-gray-100">
                                    <tr>
                                        <td colspan="4" class="px-6 py-3 text-right text-sm font-bold text-gray-800">
